...
Total_Score_MEN_NHANES <- Clean_BP_MEN_Merge %>% 
  mutate(Total_Score = rowSums(across(c(DPQ010, DPQ020, DPQ030, DPQ040, DPQ050, DPQ060, DPQ070, DPQ080, DPQ090, DPQ100)), na.rm = TRUE)) %>% 
...
BP_MEN_draft <- Total_Score_MEN_NHANES %>% 
  select(svy_id, svy_year, demo_age_cat, demo_race,
...
## Filtering for Hypertensive Individuals
```{r}
...
## Differences in Category Per Year for htn_accaha
...
BP_MEN_GLM_htn_accaha <- glm(htn_accaha ~ Category + svy_year, data = unfiltered_df, family = binomial)
...
## Differences in Category Per Year for bp_uncontrolled_accaha
...
BP_MEN_GLM_bp_uncontrolled_accaha <- glm(bp_uncontrolled_accaha ~ Category + svy_year, data = BP_MEN_Select, family = binomial)
...
```
### Multiple Logistic Regression model for bp_uncontrolled_accaha ~ All Demographic Variables and Comorbidities

```{r}

BP_MEN_GLM_bp_uncontrolled_multiple <- glm(bp_uncontrolled_accaha ~ Category + svy_year + demo_age_cat + demo_gender + cc_bmi + cc_diabetes + cc_ckd, data = BP_MEN_Select, family = binomial)
...
model1 <- glm(bp_uncontrolled_accaha ~ Category + Antidepressant_Y_N + Category*Antidepressant_Y_N, data = BP_MEN_Select, family = binomial)
summary(model1)
```
...
BP_MEN_GLM_bp_uncontrolled_accaha_demo_age_cat <- glm(bp_uncontrolled_accaha ~ Category + svy_year + demo_age_cat, data = BP_MEN_Select, family = binomial)
...
BP_MEN_GLM_bp_uncontrolled_accaha_demo_gender <- glm(bp_uncontrolled_accaha ~ Category + svy_year + demo_gender, data = BP_MEN_Select, family = binomial)
...
BP_MEN_GLM_htn_accaha_cc_bmi <- glm(bp_uncontrolled_accaha ~ Category + svy_year + cc_bmi, data = BP_MEN_Select, family = binomial)
...
BP_MEN_GLM_bp_uncontrolled_accaha_cc_diabetes <- glm(bp_uncontrolled_accaha ~ Category + svy_year + cc_diabetes, data = BP_MEN_Select, family = binomial)
...
BP_MEN_GLM_bp_uncontrolled_accaha_cc_ckd <- glm(bp_uncontrolled_accaha ~ Category + svy_year + cc_ckd, data = BP_MEN_Select, family = binomial)
